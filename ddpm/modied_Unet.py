...
#------------------------------------------#
#   Calculate positional embeddings for time steps.
#   Half are sin, half are cos.
#------------------------------------------#
...
#------------------------------------------#
#   Downsample layer, a convolution with stride 2x2
#------------------------------------------#
class Downsample(nn.Module):
    ...
    def forward(self, x, time_emb, y):
        if x.shape[2] % 2 == 1:
            print(x.shape[2] % 2,"downsampling tensor height should be even")
        if x.shape[3] % 2 == 1:
            print(x.shape[3] % 2,"downsampling tensor width should be even")

        return self.downsample(x)

#------------------------------------------#
#   Upsample layer, Upsample + Convolution
#------------------------------------------#
...
#------------------------------------------#
#   Residual structure for feature extraction
#------------------------------------------#
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None, y=None):
        ...
        # Fully connect time_emb and apply on channels
        if self.time_bias is not None:
            if time_emb is None:
                raise ValueError("time conditioning was specified but time_emb is not passed")
            out += self.time_bias(self.activation(time_emb))[:, :, None, None]

        # Fully connect class_emb and apply on channels
...
class UNet(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Activation function used, typically SILU
        self.activation = activation
        ...
        # Fully connected layer for time axis input
        ...
        # self.downs stores the layers used for downsampling. First, ResidualBlock is used for feature extraction,
        # then Downsample is used to reduce the height and width of the feature map.
        self.downs      = nn.ModuleList()
        self.ups        = nn.ModuleList()

        # channels refers to the number of channels processed by each module
        # now_channels is an intermediate variable representing the intermediate number of channels
        ...
        # Can be regarded as feature integration, an intermediate feature extraction module
        ...
        # Perform upsampling and feature fusion
    ...
    def forward(self, x, time=None, y=None):
        ...
        # skips stores intermediate layers from downsampling
        skips = [x]
        ...
        for layer in self.ups:
            if isinstance(layer, ResidualBlock):
                skip = skips.pop()
                # Check and adjust tensor sizes during the upsampling phase
                if x.shape[2] != skip.shape[2] or x.shape[3] != skip.shape[3]:
                    ...
                    x = F.pad(x, (0, diff_w, 0, diff_h)) # Pad `x` to match the `skip` size
                x = torch.cat([x, skip], dim=1) # Concatenate along the channel dimension
            x = layer(x, time_emb, y) # Pass through the current layer in the upsampling path
...
# Initialize model
model = UNet(img_channels=9) 

input_tensor = torch.randn(3, 9, 9, 9)
time_tensor = torch.randn(3)

output = model(input_tensor, time = time_tensor)
